            ...
            if reactionWillHappen:
                ...
                if KEcm < E:
                    ...
                    if DEBUG:
                        if DEBUG_VERBOSE:
                            print(
                                f"[-] NOT ENOUGH ENERGY. Particle at {t}s with: (vxcm {vxcm}), (vycm {vycm}), (vzcm {vzcm}), (KEcm {KEcm})")
                        else:
                            print(f"[-] [t = {round(t, 2)}] NOT ENOUGH ENERGY.")
                if reactionWillHappen:
                    reactions += 1.0
                    print(
                        f"[+] [t = {round(t, 2)}] #{int(reactions)} REACTION HAPPENED at {t}s, with: (vxcm {vxcm}), (vycm {vycm}), (vzcm {vzcm}), (KEcm {KEcm})")
                pass
            pass
        pass
...
print(
    f'\tNot Reacting ({len(record.get("stats_err_noenergy")) + len(record.get("stats_err_distfar"))}) ({(len(record.get("stats_err_noenergy")) + len(record.get("stats_err_distfar"))) / len(record.get("random1|2"))*100}%):')
print(
    f'\t\tDistance Too Far: {len(record.get("stats_err_distfar"))} ({round(100*(len(record.get("stats_err_distfar")) / (len(record.get("stats_err_noenergy")) + len(record.get("stats_err_distfar")))), 3)}%)')
print(
    f'\t\tEnergy Not Enough: {len(record.get("stats_err_noenergy"))} ({round(100*(len(record.get("stats_err_noenergy")) / (len(record.get("stats_err_noenergy")) + len(record.get("stats_err_distfar")))), 3)}%)')
